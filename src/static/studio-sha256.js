// Small synchronous SHA-256 primitive shared by schema validation and the
// runtime document layer. Keeping this module dependency-free avoids pulling
// the runtime document graph into low-level project validation.
...
function byteView(source) {
  ...
  throw new TypeError('SHA-256 input must be an ArrayBuffer or byte view.');
}

export function studioSha256BytesHex(source) {
  const input = byteView(source);
...
export function studioSha256TextHex(source) {
  if (typeof source !== 'string') throw new TypeError('SHA-256 text input must be a string.');
  return studioSha256BytesHex(new TextEncoder().encode(source));
}
