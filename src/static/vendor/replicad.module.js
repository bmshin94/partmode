...
function makeProjectedEdges(shape, camera, withHiddenLines = true) {
  ...
  const ordinaryVisible = getEdges(hlrShapes.VCompound_1());
  const outlineVisible = getEdges(hlrShapes.OutLineVCompound_1());
  const regularVisible = [...ordinaryVisible, ...outlineVisible];
  const tangentVisible = getEdges(hlrShapes.Rg1LineVCompound_1());
  const visible = [...ordinaryVisible, ...tangentVisible, ...outlineVisible];
  visible.forEach((e) => oc.BRepLib.BuildCurves3d_2(e.wrapped));
  const ordinaryHidden = withHiddenLines ? getEdges(hlrShapes.HCompound_1()) : [];
  const outlineHidden = withHiddenLines ? getEdges(hlrShapes.OutLineHCompound_1()) : [];
  const regularHidden = [...ordinaryHidden, ...outlineHidden];
  const tangentHidden = withHiddenLines ? getEdges(hlrShapes.Rg1LineHCompound_1()) : [];
  const hidden = [...ordinaryHidden, ...tangentHidden, ...outlineHidden];
  hidden.forEach((e) => oc.BRepLib.BuildCurves3d_2(e.wrapped));
  return { visible, hidden, regularVisible, regularHidden, tangentVisible, tangentHidden };
}
...
function drawProjectionWithEdgeClasses(shape, projectionCamera = "front") {
  let camera;
  if (projectionCamera instanceof ProjectionCamera) {
    camera = projectionCamera;
  } else {
  ...
  const { visible, hidden, regularVisible, regularHidden, tangentVisible, tangentHidden } = makeProjectedEdges(shape, camera);
  const drawingOrNull = (edges) => edges.length ? edgesToDrawing(edges) : null;
  return {
    visible: drawingOrNull(visible),
    hidden: drawingOrNull(hidden),
    regularVisible: drawingOrNull(regularVisible),
    regularHidden: drawingOrNull(regularHidden),
    tangentVisible: drawingOrNull(tangentVisible),
    tangentHidden: drawingOrNull(tangentHidden)
  };
}
